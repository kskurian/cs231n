...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    dscores = np.divide(scores, sum.reshape(N,1))
    dscores[range(scores.shape[0]),y] = -1 * ((sum - correct)/ sum)
    dscores /= N
    dW2 = H1.T.dot(dscores)
    db2 = np.sum(dscores,axis=0)
    
    tmp = W2.dot(dscores.T)
    tmp[H1.T == 0] = 0
    dW1 = tmp.dot(X)
    dW1 = dW1.T
    db1 = np.sum(tmp,axis = 1)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      #########################################################################
      grads['W2'] -= learning_rate*grads['W2']
      grads['b2'] -= learning_rate*grads['b2']
      grads['W1'] -= learning_rate*grads['W1']
      grads['b1'] -= learning_rate*grads['b1']
      #########################################################################
      #                             END OF YOUR CODE                          #
      #########################################################################
